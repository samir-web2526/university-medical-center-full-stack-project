    ...
    <div className="min-h-screen bg-slate-50 py-10 px-4">
        ...
        <Card className="border-0 shadow-md">
          ...
          <CardContent className="pt-5 space-y-4">
            ...
            <div className="flex items-center gap-3">
              <Button
                size="sm"
                onClick={() => openStockDialog("increase")}
              ...
              <Button
                size="sm"
                variant="outline"
                onClick={() => openStockDialog("decrease")}
                disabled={isOut}
          ...
          <Card className="border-0 shadow-md">
            ...
            <CardContent className="space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-600">
                  {new Date(med.updatedAt).toLocaleDateString("en-US", {
                    year: "numeric", month: "long", day: "numeric",
                  })}
                </p>
              </div>
        ...
        <Card className="border-0 shadow-md border-l-4 border-l-red-500">
          <CardContent className="py-5 px-6">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="text-sm font-semibold text-slate-800">Delete Medicine</h3>
      ...
      <Dialog open={!!stockAction} onOpenChange={(o) => !o && setStockAction(null)}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {stockAction === "increase"
                ? <><TrendingUp className="w-4 h-4 text-emerald-600" />Add Stock</>
                : <><TrendingDown className="w-4 h-4 text-orange-600" />Remove Stock</>}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            <div className="bg-slate-50 rounded-lg p-3 text-sm text-slate-700">
              <span className="font-semibold">{med.name}</span>
              <span className="text-slate-400 ml-2">Current: {med.stockQuantity}</span>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setStockAction(null)} disabled={isAdjusting}>
              Cancel
            </Button>
            <Button
              onClick={handleStockAdjust}
              disabled={isAdjusting}
              className={stockAction === "increase" ? "bg-emerald-600 hover:bg-emerald-700" : "bg-orange-500 hover:bg-orange-600"}
            >
              {isAdjusting
